...
class DoublyLinkedList:
    ...
    def reverse(self):
        temp = self.head
        while temp is not None:
            temp.prev, temp.next = temp.next, temp.prev # swap the prev and next pointers of node points to
            temp = temp.prev # move to the next node
        self.head, self.tail = self.tail, self.head  # swap the head and tail pointers
